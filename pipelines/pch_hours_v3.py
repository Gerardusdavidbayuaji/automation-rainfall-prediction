...
mask_pulau = "repository/input/data_vektor/pulau.geojson"
...
output_masked = "repository/output/hours/idw_masked"
...
def process_netcdf(local_file_path):
    ...
    today_for_prediction = datetime.date.today()
    three_days_prediction = today_for_prediction + datetime.timedelta(days=3)

    df_hour_prediction = df_accumulated[(df_accumulated['time'].dt.date >= today_for_prediction) & (df_accumulated['time'].dt.date <= three_days_prediction)]
    
    # cek tanggal minimal dan maksimal data .nc yang sedang diproses
    min_date = df_hour_prediction['time'].min()
    max_date = df_hour_prediction['time'].max()

    print(f"Data range: {min_date} to {max_date}")

    return df_hour_prediction

# Ubah data nc menjadi csv, untuk dilanjutkan interpolasi IDW 
def save_to_csv(df_hour_prediction, output_nc_to_csv):
    unique_times = df_hour_prediction['time'].unique()

    for unique_time in unique_times:
        filtered_df = df_hour_prediction[df_hour_prediction['time'] == unique_time].copy()
...
def interpolate_and_save_to_tiff(df_hour_prediction, output_csv_to_idw, mask_pulau, output_masked):
    unique_times = df_hour_prediction['time'].unique()

    gdf = gpd.read_file(mask_pulau)
    ...
    for unique_time in unique_times:
        ...
        result_masked = f"{output_masked}/pch_hour_{time_str}.tif"
        with rasterio.open(result_masked, "w", **out_meta) as dest:
            dest.write(out_image)

        print(f"Hasil masking disimpan di: {result_masked}")

        # Upload to GeoServer
        store_name = os.path.splitext(os.path.basename(result_masked))[0]
        upload_to_geoserver(result_masked, store_name)
...
def process_netcdf_and_interpolate_with_extraction(
    ...
    df_hour_prediction = process_netcdf(local_file_path)
    save_to_csv(df_hour_prediction, output_nc_to_csv)
    
    # Langkah 2: Interpolasi dan Simpan GeoTIFF
    interpolate_and_save_to_tiff(df_hour_prediction, output_csv_to_idw, mask_pulau, output_masked)

...
output_masked = "repository/output/daily/idw_masked"
...
def process_netcdf(local_file_path):
    ...
    df_daily_prediction = df_daily[(df_daily['time'].dt.date >= today_for_prediction) & (df_daily['time'].dt.date <= three_days_prediction)]
    
    # cek tanggal minimal dan maksimal data .nc yang sedang diproses
    min_date = df_daily_prediction['time'].min()
    max_date = df_daily_prediction['time'].max()

    print(f"Data range: {min_date} to {max_date}")

    return df_daily_prediction

# Ubah data nc menjadi csv, untuk dilanjutkan interpolasi IDW 
def save_to_csv(df_daily_prediction, output_nc_to_csv):
    unique_times = df_daily_prediction['time'].unique()

    for unique_time in unique_times:
        filtered_df = df_daily_prediction[df_daily_prediction['time'] == unique_time].copy()
...
def interpolate_and_save_to_tiff(df_daily_prediction, output_csv_to_idw, mask_pulau, output_masked):
    unique_times = df_daily_prediction['time'].unique()
...
def process_netcdf_and_interpolate_with_extraction(
    ...
    df_daily_prediction = process_netcdf(local_file_path)
    save_to_csv(df_daily_prediction, output_nc_to_csv)
    
    # Langkah 2: Interpolasi dan Simpan GeoTIFF
    interpolate_and_save_to_tiff(df_daily_prediction, output_csv_to_idw, mask_pulau, output_masked)
